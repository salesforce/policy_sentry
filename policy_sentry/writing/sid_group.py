...
class SidGroup:
    ...
    def add_by_arn_and_access_level(self, db_session, arn_list, access_level):
        for arn in arn_list:
            ...
            for serv in service_action_data:
                for row in service_action_data[serv]:
                    if does_arn_match(arn, row["resource_arn_format"]) and row["access_level"] == access_level:
                        ...
                        actions = get_actions_with_arn_type_and_access_level(db_session, serv, resource_type_name, access_level)
                        temp_sid_dict = {
                            'arn': [arn],
